...
import dev.nick.app.screencast.app.RecorderActivity;
...
public class ScreencastService extends Service implements IScreencaster, Handler.Callback {
    ...
    void cleanup() {
        ...
        if (recorderPath != null) {
            ...
            message.what = 1;
            message.obj = recorderPath;
            RecorderActivity.mhandle.sendMessage(message);
//            sendShareNotification(recorderPath);
    ...
    public void updateNotification(Context context) {
        ...
        String vl = "videolength"+DateUtils.formatElapsedTime(timeElapsed / 1000);
        mBuilder.setContentText(vl);
    ...
    protected Point getNativeResolution() {
        ...
        boolean landscape = current == Configuration.ORIENTATION_LANDSCAPE; //SettingsProvider.get().orientation() == Orientations.L;
        int width, height;
//        int preferredResIndex = 5;//SettingsProvider.get().resolutionIndex();
//        if (preferredResIndex != ValidResolutions.INDEX_MASK_AUTO) {
//
//        }
        int[] resolution ={1920,1080};// ValidResolutions.$[preferredResIndex];
        if (landscape) {
            width = resolution[0];
            height = resolution[1];
        } else {
    ...
    private void stopCasting() {
        ...
        if (!hasAvailableSpace()) {
            Toast.makeText(this, "insufficient_storage", Toast.LENGTH_LONG).show();
        }
        mIsCasting = false;
        notifyUncasting();
    }
...
//    }
    ...
    public boolean start(final MediaProjection projection, final boolean withAudio) {
        if (!hasAvailableSpace()) {
            Toast.makeText(this, "not_enough_storage", Toast.LENGTH_LONG).show();
            return false;
        }
        ...
        }, 0);//SettingsProvider.get().startDelay());
        return true;
    }
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what) {
            case SENSOR_SHAKE:
                boolean shouldHandle = false;//SettingsProvider.get().shakeAction();
